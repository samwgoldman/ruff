...
This test suite explores the interplay between generics and set theoretic gradual types. We use PEP
695 generics for demonstration, so we set the Python version to 3.12 and import some utilities:
...
be accessed on that type, which seems undesirable.

Instead, most users probably interpret `Invariant[Any]` as "an instance of `Invariant` with an
unknown specialization". However, a simple "you are allowed to replace `Any` with an arbitrary type"
